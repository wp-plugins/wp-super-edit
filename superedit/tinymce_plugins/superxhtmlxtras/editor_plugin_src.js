...
tinyMCE.importPluginLanguagePack('superxhtmlxtras');
	...
	execCommand : function(editor_id, element, command, user_interface, value) {
		...
		switch (command) {
			case "mceCite":
				...
				template = new Array();
				template['file'] = this.baseURL + '/cite.htm';
				template['width'] = 350;
				template['height'] = 250;
			...
			case "mceAcronym":
				...
				template = new Array();
				template['file'] = this.baseURL + '/acronym.htm';
				template['width'] = 350;
				template['height'] = 250;
				tinyMCE.openWindow(template, {editor_id : editor_id});
				return true;
			...
			case "mceAbbr":
				...
				template = new Array();
				template['file'] = this.baseURL + '/abbr.htm';
				template['width'] = 350;
				template['height'] = 250;
				tinyMCE.openWindow(template, {editor_id : editor_id});
				return true;
			...
			case "mceIns":
				...
				template = new Array();
				template['file'] = this.baseURL + '/ins.htm';
				template['width'] = 350;
				template['height'] = 310;
				tinyMCE.openWindow(template, {editor_id : editor_id});
				return true;
			...
			case "mceDel":
				...
				template = new Array();
				template['file'] = this.baseURL + '/del.htm';
				template['width'] = 350;
				template['height'] = 310;
				tinyMCE.openWindow(template, {editor_id : editor_id});
				return true;
			...
			case "mceAttributes":
				...
				if (elm && elm.nodeName !== 'BODY' && elm.className.indexOf('mceItem') == -1) {
					tinyMCE.openWindow({
						file : this.baseURL + '/attributes.htm',
						width : 380,
						height : 370
					}, {editor_id : editor_id});
				}
...
tinyMCE.addPlugin("superxhtmlxtras", TinyMCE_XHTMLXtrasPlugin);
